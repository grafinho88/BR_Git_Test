...
function (EventHandler, ClientSystemEvent, BreaseEvent, SystemGestures, Utils, ContentStatus, _visuModel, Areas) {
    ...
    var EventController = {
            ...
            getAreaForPoint: function (point) {
                ...
                for (var areaId in cp.assignments) {
                    var area = Areas.getArea(brease.appElem.id, cp.layout, areaId),
                        rect = area.div.getBoundingClientRect();
                ...
                if (areas.length > 1) {
                    areas.sort(function (a, b) { return b.info.zIndex - a.info.zIndex; });
                }
                if (areas.length > 0) {
                    return areas[0].info.id;
                } else {
                    return undefined;
                }
            }
